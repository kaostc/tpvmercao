...
/**
 * \file    test/phpunit/TPVMercaoFunctionalTest.php
 * \ingroup tpvmercao
 ...
 *
 * @package Testtpvmercao
 */
class TPVMercaoFunctionalTest extends \PHPUnit_Extensions_Selenium2TestCase
{
	...
	public function testModuleEnabled()
	{
		...
		if (strstr($module_status_image->attribute('src'), 'switch_off.png')) {
			// Enable the module
			$this->byHref('modTPVMercao')->click();
		} else {
			// Disable the module
			$this->byHref('modTPVMercao')->click();
			// Reenable the module
	...
	{
		$this->url('/custom/tpvmercao/admin/setup.php');
		$this->authenticate();
		return $this->assertContains('tpvmercao/admin/setup.php', $this->url(), 'Configuration page');
	}
	...
	{
		$this->url('/custom/tpvmercao/admin/about.php');
		$this->authenticate();
		return $this->assertContains('tpvmercao/admin/about.php', $this->url(), 'About page');
	}
	...
	public function testBoxDeclared()
	{
		...
		$this->authenticate();
		return $this->assertContains('tpvmercaowidget1', $this->source(), "Box enabled");
	}
	...
	public function testTriggerDeclared()
	{
		...
		return $this->assertContains(
			'interface_99_modTPVMercao_TPVMercaoTriggers.class.php',
	...
	public function testTriggerEnabled()
	{
		...
		return $this->assertContains(
			'tick.png',
			$this->byXPath('//td[text()="interface_99_modTPVMercao_MyTrigger.class.php"]/following::img')->attribute('src'),
			"Trigger enabled"
		);
	}
